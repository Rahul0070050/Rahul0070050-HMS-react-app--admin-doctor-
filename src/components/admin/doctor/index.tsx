import React from 'react'
import { Button, Card, CardActions, CardMedia, Typography } from '@mui/material'

import './style.scss'

function Doctor() {
  return (
    <Card sx={{ m: '2rem 0 0 0', p: '1rem', display: 'flex' }} className='single-doctor'>
      <img src="data:image/jpeg;base64," alt="" />
      <div className="info">
        <div className="key">
          <Typography>ID: </Typography>
          <Typography>Name: </Typography>
          <Typography>Department: </Typography>
          <Typography>Joind: </Typography>
          <Typography>Salary: </Typography>
          <CardActions>
            <Button>View</Button>
            <Button>Message</Button>
          </CardActions>
        </div>
        <div className="connect">
          <Typography>:</Typography>
          <Typography>:</Typography>
          <Typography>:</Typography>
          <Typography>:</Typography>
          <Typography>:</Typography>
        </div>
        <div className="value">
          <Typography>234567899</Typography>
          <Typography>Rahul</Typography>
          <Typography>Department 1</Typography>
          <Typography>32/12/1999</Typography>
          <Typography>400000</Typography>
        </div>
      </div>
    </Card>
  )
}

export default Doctor
